...
public:
    ...
    bool esVacia();
    bool buscar(T elem);
    bool comparar(T elem, ulong pos, function<bool(T, T)> f);
    void invertir();
    void rotar(ulong n, bool iniFin);
};
...
template <typename T>
void ListaDE<T>::insPos(T elem, ulong pos)
{
    if (pos >= 0 && pos < lon)
    {
        if (pos == 0)
...
template <typename T>
void ListaDE<T>::elimPos(ulong pos)
{
    if (pos >= 0 && pos < lon)
    {
        if (pos == 0)
...
template <typename T>
void ListaDE<T>::modPos(T elem, ulong pos)
{
    if (pos >= 0 && pos < lon)
    {
        if (pos == 0)
...
// Otras
template <typename T>
ulong ListaDE<T>::longitud() { return lon; }
...
template <typename T>
bool ListaDE<T>::comparar(T elem, ulong pos, function<bool(T, T)> f)
{
    if (ini != nullptr)
...
template <typename T>
void ListaDE<T>::rotar(ulong n, bool iniFin)
{
